...
## 22.5 第十四阶段：控制台残留修复 + 桌宠尺寸 + 透明边缘 + UI 主题

### 22.5.1 控制台窗口残留（根因 + 修复）

**根因（已定位）**：
1. `pet-agent` 缺 `#![windows_subsystem = "windows"]` → 编译为 **console 子系统**，
   每次运行（ui / tts-test / 任意子命令）都弹出属于它自己的控制台窗口，进程存活期间不关闭 → 残留。
2. `tts/piper.rs` 用 `Command::new(python.exe)` 且未加 `CREATE_NO_WINDOW`，
   父进程是 console → 每次 TTS synthesis 弹 cmd。

**修复**：
- pet-agent 加 `#![windows_subsystem = "windows"]`；
- 新增 `attach_parent_console()`：仅当 stdout 句柄无效时才 `AttachConsole(ATTACH_PARENT_PROCESS)`
  并重开 `CONOUT# DesktopPet — Live2D 桌宠 Runtime
...
（保证 CLI 从终端运行仍可输出，双击不弹窗）；
- Piper `Command` 加 `creation_flags(CREATE_NO_WINDOW = 0x08000000)`。

**验证**：30×piper-check / 30×tts-test / 30×provider-test / 20×ui 后
`python=0`、`agent` 保持单实例、cmd 无增长；全部关闭后 `agent=0 python=0`。

### 22.5.2 桌宠尺寸

- 默认窗口 640×640 → **360×360**；character scale 1.0 → **0.5**。
- 实测窗口 360×360，hit test 中心命中 / 透明角穿透正常；气泡定位与 look/嘴型沿用既有逻辑。

### 22.5.3 头发边缘 / 透明

- 贴图 `texture_00.png` 检查：RGBA，背景 alpha=0（采样 57.6% 透明）→ **贴图本身干净**。
- 根因：atlas 是 **straight alpha**，但 shim 设 `IsPremultipliedAlpha(true)`，
  premultiplied shader 把 straight 贴图当预乘 → 发丝边缘暗/白边。
- 修复：上传纹理时把 **RGB 预乘 alpha**（`rgb = rgb * a / 255`）。
- 结果：需人工目视确认发丝边缘是否干净（见报告）。

### 22.5.4 UI 主题

- 新增 `theme.rs`：深色背景/面板/文字色、Microsoft YaHei UI 字体、`WM_CTLCOLOR*` 处理。
- 输入窗口 + Memory Manager 窗口应用深色背景 + 中文主题字体。
